...
packageResults <- function(connectionDetails, cdmDatabaseSchema, outputFolder, minCellCount = 5) {
  ...
  cmAnalysisList <- CohortMethod::loadCmAnalysisList(system.file("settings", "cmAnalysisList.json", package = "COX2vsNSAIDsAKI"))
...
createMetaData <- function(connectionDetails, cdmDatabaseSchema, exportFolder) {
  ...
  snapshot <- OhdsiRTools::takeEnvironmentSnapshot("COX2vsNSAIDsAKI")
  lines <- c(lines, "")
