...
plotForest <- function(subset, row) {
  ...
  plot <- ggplot2::ggplot(subset, ggplot2::aes(x = rr, 
    ...
    ggplot2::geom_errorbarh(height = 0.2, alpha = 1, size = 1, color = rgb(0,0,0), data = highlight, show.legend = FALSE) + 
    ggplot2::geom_point(shape = 16, size = 3, color = rgb(0,0,0), alpha = 1, data = highlight, show.legend = FALSE) + 
