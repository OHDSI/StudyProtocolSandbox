...
WHERE C.condition_start_date <= DATEFROMPARTS(2005, 12, 30)
...
WHERE DATEADD(day,@window_period_days,OP.OBSERVATION_PERIOD_START_DATE) <= P.START_DATE AND DATEADD(day,0,P.START_DATE) <= OP.OBSERVATION_PERIOD_END_DATE AND P.ordinal = 1
-- End Primary Events
...
) A on A.person_id = P.person_id and A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,-@window_period_days,P.START_DATE) and A.START_DATE <= DATEADD(day,-1,P.START_DATE)
