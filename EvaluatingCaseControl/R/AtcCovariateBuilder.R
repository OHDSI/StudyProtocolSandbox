...
getDbAtcCovariateData <- function(connection,
  ...
  for (i in 1:nrow(covariateSettings$covariateDefs)) {
    sql <- SqlRender::loadRenderTranslateSql("atcToConcepts.sql",
                                             packageName = "EvaluatingCaseControl",
                                             dbms = attr(connection, "dbms"),
                                             oracleTempSchema = oracleTempSchema,
                                             covariate_id = covariateSettings$covariateDefs$covariateId[i],
                                             atc = covariateSettings$covariateDefs$atc[i],
                                             cdm_database_schema = cdmDatabaseSchema)
    DatabaseConnector::executeSql(connection, sql, progressBar = FALSE, reportOverallTime = FALSE)
  }
  sql <- SqlRender::loadRenderTranslateSql("getAtcCovariates.sql",
                                           packageName = "EvaluatingCaseControl",
                                           dbms = attr(connection, "dbms"),
                                           oracleTempSchema = oracleTempSchema,
