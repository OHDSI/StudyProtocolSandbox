...
#
# This file is part of Existing Bleeding Risk External Valiation study
#
...
createCohorts <- function(connectionDetails,
                          ...
                          cohortTable){
  targetId = c(1113,1115,1116,1114,1117,1118)
  outcomeIds = c(1028,1029,1036)

  cohortDetails <- NULL

  cohortDetails <- data.frame(cohortName=c('Atrial fibrillation general first year',
                                             'Atrial fibrillation general with warfarin',
                                           'Atrial fibrillation general with NOAC',
                                           'Atrial fibrillation older than 75 first year',
                                           'Atrial fibrillation older than 75 with warfarin',
                                           'Atrial fibrillation older than 75 with NOAC',
  ...
  if(!existTab){
    sql <- SqlRender::loadRenderTranslateSql("createTable.sql",
                                             packageName = "ExistingBleedingValidationInAF",
  ...
  result <- PatientLevelPrediction::createCohort(connectionDetails = connectionDetails,
                                       ...
                                       package = 'ExistingBleedingValidationInAF')
  } else {
...
applyExistingstrokeModels <- function(connectionDetails,
  ...
  writeLines('Implementing HasBled stroke risk model...')
  hasBled <- PredictionComparison::hasBledModel(connectionDetails, cdmDatabaseSchema,
                                                    cohortDatabaseSchema = cohortDatabaseSchema,
                                                    outcomeDatabaseSchema = cohortDatabaseSchema,
                                                    cohortTable = cohortTable,
                                                    outcomeTable = cohortTable,
