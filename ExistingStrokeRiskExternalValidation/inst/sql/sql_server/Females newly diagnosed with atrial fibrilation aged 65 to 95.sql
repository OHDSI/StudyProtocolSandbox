...
LEFT JOIN
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (4108832)and invalid_reason is null
...
WHERE C.ordinal = 1
AND (YEAR(C.condition_start_date) - P.year_of_birth >= 65 and YEAR(C.condition_start_date) - P.year_of_birth <= 95)
...
WHERE DATEADD(day,730,OP.OBSERVATION_PERIOD_START_DATE) <= P.START_DATE AND DATEADD(day,0,P.START_DATE) <= OP.OBSERVATION_PERIOD_END_DATE AND P.ordinal = 1
-- End Primary Events
