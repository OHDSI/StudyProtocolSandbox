...
shinyServer(function(input, output) {
  ...
  tableSubset <- reactive({
    idx <- resultsHois$comparison == input$comparison &
      ...
      resultsHois$description %in% input$analysis &
  ...
  balance <- reactive({
    ...
    if (is.null(row)) {
      ...
      fileName <- file.path(studyFolder, row$database, paste0("balance_a",row$analysisId,"_t",row$targetId,"_c",row$comparatorId, ".csv"))
  ...
  table1 <- reactive({
    ...
    if (is.null(row)) {
      ...
      fileName <- file.path(studyFolder, row$database, paste0("table1_a", row$analysisId, "_t", row$targetId, "_c", row$comparatorId, ".csv"))
  ...
  output$psPlot <- renderPlot({
    row <- selectedRow()
    if (!is.null(row)) {
      fileName <- file.path(studyFolder, row$database, paste0("preparedPsPlot_a",row$analysisId,"_t",row$targetId,"_c",row$comparatorId,".csv"))
      data <- read.csv(fileName)
      data$GROUP <- row$targetName
      data$GROUP[data$treatment == 0] <- row$comparatorName
      data$GROUP <- factor(data$GROUP, levels = c(row$targetName, 
                                                  row$comparatorName))
  ...
  output$negativeControlPlot <- renderPlot({
    row <- selectedRow()
    if (!is.null(row)) {
      ncs <- resultsControls[resultsControls$targetId == row$targetId & 
                               resultsControls$comparatorId == row$comparatorId & 
                               resultsControls$analysisId == row$analysisId &
                               resultsControls$database == row$database &
                               resultsControls$targetEffectSize == 1, ]
      # null <- EmpiricalCalibration::fitMcmcNull(ncs$logRr, ncs$seLogRr)
      fileName <- file.path(studyFolder, row$database, paste0("null_a",row$analysisId,"_t",row$targetId,"_c",row$comparatorId,".rds"))
      if (file.exists(fileName)) {
  ...
  output$hoverInfoForestPlot <- renderUI({
    ...
    div(
      ...
      wellPanel(
        style = style,
        p(HTML(paste0("<b> Analysis: </b>", point$description, "<br/>",
