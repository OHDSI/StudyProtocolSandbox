SELECT 'Gender, Female', COUNT(c.gender_concept_id), Null, Null, Null, Null, Null, Null, Null FROM (
	SELECT DISTINCT ON (b.subject_id) b.subject_id, b.gender_concept_id FROM 
		(SELECT subject_id, gender_concept_id FROM 
			...
			) a WHERE cohort_definition_id=2 or cohort_definition_id=3
		GROUP BY subject_id, gender_concept_id) b
	) c WHERE c.gender_concept_id = 8532
