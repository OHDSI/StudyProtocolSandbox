SELECT 'Lipid Disorder', COUNT(*), Null, Null, Null, Null, Null, Null, Null FROM (
	SELECT DISTINCT c.person_id FROM
		(SELECT * FROM
			...
			LEFT JOIN @cdm_database_schema.concept_ancestor ON (condition_concept_id=concept_ancestor.descendant_concept_id) WHERE cohort_definition_id=2 or cohort_definition_id=3) b
		WHERE b.condition_start_date <= b.cohort_start_date
		)c
	WHERE c.ancestor_concept_id = 432867 or c.ancestor_concept_id = 4079888) d 
