SELECT 'Race Ethnicity African American', COUNT(d.subject_id), Null, Null, Null, Null, Null, Null, Null FROM (
	SELECT DISTINCT ON (c.subject_id) c.subject_id, c.eth_race FROM
		...
		END  AS eth_race FROM
			(SELECT a.subject_id, a.ethnicity_concept_id, a.race_concept_id FROM 
				...
				WHERE cohort_definition_id=2 or cohort_definition_id=3
