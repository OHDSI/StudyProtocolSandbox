SELECT 'Index Event Unstable Angina wo STEMI NSTEMI', COUNT(*), Null, Null, Null, Null, Null, Null, Null FROM (
	SELECT DISTINCT c.person_id FROM
		(SELECT * FROM
			...
			LEFT JOIN @cdm_database_schema.concept_ancestor ON (condition_concept_id=concept_ancestor.descendant_concept_id) WHERE cohort_definition_id=0) b
		WHERE b.condition_start_date = b.cohort_start_date
		)c
	WHERE c.ancestor_concept_id = 35205182  
	...
	WHERE c.condition_concept_id = 312327 or c.condition_concept_id = 444406 ) d
