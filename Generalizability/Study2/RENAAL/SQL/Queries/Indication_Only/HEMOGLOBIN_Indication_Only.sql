...
SELECT 'Hemoglobin', Null, MIN(c.indiv_mean), percentile_disc(0.25) WITHIN GROUP (ORDER BY c.indiv_mean), MEDIAN(c.indiv_mean), percentile_disc(0.75) WITHIN GROUP (ORDER BY c.indiv_mean), 
MAX(c.indiv_mean), AVG(c.indiv_mean), STDDEV(c.indiv_mean) FROM (
	...
	WHERE b.measurement_concept_id = 3000963   GROUP BY b.person_id ) c 
