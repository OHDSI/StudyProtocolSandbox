...
SELECT 'Antihypertensive Drugs', COUNT(*), Null, Null, Null, Null, Null, Null, Null FROM (
	SELECT DISTINCT c.person_id FROM
		(SELECT * FROM
			...
			LEFT JOIN @cdm_database_schema.concept_ancestor ON (A.drug_concept_id=concept_ancestor.descendant_concept_id) WHERE cohort_definition_id=0) b
		WHERE b.drug_exposure_start_date <= b.cohort_start_date
		)c
	WHERE c.ancestor_concept_id = 21600381 ) d 
