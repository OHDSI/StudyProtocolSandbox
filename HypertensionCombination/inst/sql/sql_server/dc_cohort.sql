...
( 
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (1319998,1314002,1322081,1338005,950370,1346823,19049145,19063575,1386957,1307046,1314577,19024904,1327978,1345858,1353766)and invalid_reason is null
...
( 
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (1395058,974166,978555,907013,19010493)and invalid_reason is null
...
( 
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (319844,312327,434376,438170,444406,316427,438168,321318,438172,435561,315286,317576,4127089,4108215,4110961,4108678,314666,4119953,319038,315296,4108679,4108219,4108220,4124683,4108217,4108677,4108218,4108680)and invalid_reason is null
UNION  select c.concept_id
  from @cdm_database_schema.CONCEPT c
  join @cdm_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (319844,312327,434376,438170,444406,316427,438168,321318,438172,435561,315286,317576,4127089,4108215,4110961,4108678,314666,4119953,319038,315296,4108679,4108219,4108220,4124683,4108217,4108677,4108218,4108680)
  and c.invalid_reason is null
...
UNION  select c.concept_id
  from @cdm_database_schema.CONCEPT c
  join @cdm_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (319835,316139,314378,439846)
  and c.invalid_reason is null
...
( 
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (4319328,4326561,4218781,376713,4110189,443454,4111714,4108356,4110190,4110192,381316,4176892,4110185,436430,4111709,4226021,432923,4108952,4111708,4049659)and invalid_reason is null
UNION  select c.concept_id
  from @cdm_database_schema.CONCEPT c
  join @cdm_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (4319328,4326561,4218781,376713,4110189,443454,4111714,4108356,4110190,4110192,381316,4176892,4110185,436430,4111709,4226021,432923,4108952,4111708,4049659)
  and c.invalid_reason is null
...
( 
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (1332418,19089969,1328165,1353776,1326012,19004539,19015802,19071995,19102106,1318137,1318853,19113063,1319133,1319880,19020061,1307863)and invalid_reason is null
...
( 
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (1335471,1351557,1340128,19050216,1341927,1346686,1363749,19122327,1347384,1308216,1367500,1310756,40226742,1373225,1331235,1334456,1317640,1342439,1308842,19102107)and invalid_reason is null
...
(
  ...
  where co.condition_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 16)
) C
...
) A on A.person_id = P.person_id and A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,0,P.START_DATE) and A.START_DATE <= DATEADD(day,30,P.START_DATE) AND A.END_DATE >= P.OP_START_DATE AND A.END_DATE <= P.OP_END_DATE
GROUP BY p.person_id, p.event_id
HAVING COUNT(A.TARGET_CONCEPT_ID) >= 1
-- End Correlated Criteria
