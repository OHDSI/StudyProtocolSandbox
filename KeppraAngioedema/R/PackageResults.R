...
packageResults <- function(connectionDetails, cdmDatabaseSchema, outputFolder) {
    ...
    cohortMethodDataFolder <- outcomeReference$cohortMethodDataFolder[outcomeReference$analysisId == 3 & outcomeReference$outcomeId == 3]
    ...
    strataFile <- outcomeReference$strataFile[outcomeReference$analysisId == 3 & outcomeReference$outcomeId == 3]
    strata <- readRDS(strataFile)
    CohortMethod::plotPs(strata, unfilteredData = ps, fileName = file.path(exportFolder, "PsAfterVarRatioMatchingPrefScale.png"))
    CohortMethod::plotPs(strata, unfilteredData = ps, scale = "propensity", fileName = file.path(exportFolder, "PsAfterVarRatioMatching.png"))
    strataFile <- outcomeReference$strataFile[outcomeReference$analysisId == 2 & outcomeReference$outcomeId == 3]
    strata <- readRDS(strataFile)
    CohortMethod::plotPs(strata, unfilteredData = ps, fileName = file.path(exportFolder, "PsAfter1On1MatchingPrefScale.png"))
    CohortMethod::plotPs(strata, unfilteredData = ps, scale = "propensity", fileName = file.path(exportFolder, "PsAfter1On1Matching.png"))
    ...
    strataFile <- outcomeReference$strataFile[outcomeReference$analysisId == 2 & outcomeReference$outcomeId == 3]
    strata <- readRDS(strataFile)
    CohortMethod::plotKaplanMeier(strata, includeZero = FALSE, fileName = file.path(exportFolder, "KaplanMeierPerProtocol.png"))
    strataFile <- outcomeReference$strataFile[outcomeReference$analysisId == 6 & outcomeReference$outcomeId == 3]
    strata <- readRDS(strataFile)
    CohortMethod::plotKaplanMeier(strata, includeZero = FALSE, fileName = file.path(exportFolder, "KaplanMeierIntentToTreat.png"))

    ### Main outcome models ###
    outcomeModelFile <- outcomeReference$outcomeModelFile[outcomeReference$analysisId == 4 & outcomeReference$outcomeId == 3]
    ...
    if (outcomeModel$outcomeModelStatus == "OK") {
        ...
        write.csv(balance, file.path(exportFolder, "OutcomeModelPerProtocol.csv"), row.names = FALSE)
    }

    outcomeModelFile <- outcomeReference$outcomeModelFile[outcomeReference$analysisId == 8 & outcomeReference$outcomeId == 3]
    outcomeModel <- readRDS(outcomeModelFile)
    if (outcomeModel$outcomeModelStatus == "OK") {
        model <- CohortMethod::getOutcomeModel(outcomeModel, cohortMethodData)
        write.csv(balance, file.path(exportFolder, "OutcomeModelIntentToTreat.csv"), row.names = FALSE)
    }
...
#'
#' @details
#' Creates a file containing metadata about the source data (taken from the cdm_source table)
...
createMetaData <- function(connectionDetails, cdmDatabaseSchema, exportFolder) {
    ...
    lines <- c(lines, paste("DatabaseConnector version", packageVersion("DatabaseConnector"), sep = ": "))
    lines <- c(lines, paste("Cyclops version", packageVersion("Cyclops"), sep = ": "))
    lines <- c(lines, paste("FeatureExtraction version", packageVersion("FeatureExtraction"), sep = ": "))
    ...
    lines <- c(lines, paste("KeppraAngioedema version", packageVersion("KeppraAngioedema"), sep = ": "))
