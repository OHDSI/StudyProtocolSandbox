...
( 
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (4108832,313217)and invalid_reason is null
UNION  select c.concept_id
  from @cdm_database_schema.CONCEPT c
  join @cdm_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (4108832,313217)
  and c.invalid_reason is null
...
WHERE C.drug_era_start_date > DATEFROMPARTS(2010, 09, 30)
...
) A on A.person_id = P.person_id and A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,-365,P.START_DATE) and A.START_DATE <= P.OP_END_DATE
