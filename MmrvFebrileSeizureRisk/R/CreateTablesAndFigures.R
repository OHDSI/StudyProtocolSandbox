...
createTableAndFigures <- function(exportFolder) {
  ...
  ### Attrition diagrams
  attrition <- read.csv(file.path(exportFolder, "Attrition.csv"))
  object <- list()
  attr(object, "metaData") <- list(attrition = attrition)
  CohortMethod::drawAttritionDiagram(object, fileName = file.path(tablesAndFiguresFolder, "Attrition.png"))
}
