...
allCohorts <- read.csv(system.file("settings",
                                   "CohortsToCreate.csv",
                                   package = "MmrvFebrileSeizureRisk"))
...
invisible(mapply(allCohorts$cohortId, allCohorts$name, FUN = function(outcomeId, name) {
  ...
  for (file in folders) {
    if (!(file %in% skip)) {
      if (file.info(file.path(studyFolder, file))$isdir) {
        writeLines(paste("Processing", file))
        results <- read.csv(file.path(studyFolder,
                                      file,
                                      "tablesAndFigures",
...
invisible(mapply(allCohorts$cohortId, allCohorts$name, FUN = function(outcomeId, name) {
  ...
  # allResults$db[allResults$db == 'Ims_Amb_Emr'] <- 'IMS Ambulatory' allResults$db[allResults$db ==
  # 'Optum'] <- 'Optum' allResults$db[allResults$db == 'Pplus_Ims'] <- 'IMS P-Plus'
  # allResults$db[allResults$db == 'Truven_CCAE'] <- 'Truven CCAE' allResults$db[allResults$db ==
  # 'Truven_MDCD'] <- 'Truven MDCD' allResults$db[allResults$db == 'Truven_MDCR'] <- 'Truven MDCR'
  ...
  crude <- results[, c("db",
                       ...
                       "eventsTreated",
                       "eventsComparator")]
  png(file.path(studyFolder,
                paste0("countsItt_", name, ".png")), height = 30 * nrow(crude), width = 100 *
    ncol(crude))
  p <- tableGrob(crude)
  grid.arrange(p)
  dev.off()
  ...
  crude <- results[, c("db",
                       ...
                       "eventsComparator")]
  png(file.path(studyFolder,
                paste0("countsPp_", name, ".png")), height = 30 * nrow(crude), width = 100 *
    ncol(crude))
  p <- tableGrob(crude)
  grid.arrange(p)
  dev.off()
...
# meta <- metagen(results$logRr, results$seLogRr, studlab = results$db, sm = 'RR') s <-
# summary(meta)$random exp(s$TE) forest(meta) results <- allResults[allResults$analysisId == 7, ]
# results <- results[!is.na(results$seLogRr), ] meta <- metagen(results$logRr, results$seLogRr,
# studlab = results$db, sm = 'RR') forest(meta)
