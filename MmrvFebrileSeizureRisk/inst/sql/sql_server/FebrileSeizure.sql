...
( 
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (4029498,377091)and invalid_reason is null
UNION  select c.concept_id
  from @cdm_database_schema.CONCEPT c
  join @cdm_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (4029498,377091)
  and c.invalid_reason is null

) I
LEFT JOIN
(
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (4233724,380533,45757050)and invalid_reason is null
UNION  select c.concept_id
  from @cdm_database_schema.CONCEPT c
  join @cdm_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (4233724,380533,45757050)
  and c.invalid_reason is null
...
WHERE YEAR(C.condition_start_date) - P.year_of_birth <= 3
...
(	
	SELECT
		groupid
		, DATEADD(day,-1 * 42, event_date)  as end_date
	FROM
	(
		SELECT
			groupid
		...
		(
			...
			SELECT
				groupid
				, DATEADD(day,42,end_date) as end_date
				, 1 AS event_type
				, NULL
			FROM cteSource
		) RAWDATA
