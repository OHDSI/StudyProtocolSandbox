...
( 
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (377091)and invalid_reason is null
...
LEFT JOIN
(
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (4233724)and invalid_reason is null
UNION  select c.concept_id
  from @cdm_database_schema.CONCEPT c
  join @cdm_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
( 
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (2213466,46275151)and invalid_reason is null
UNION  select c.concept_id
  from @cdm_database_schema.CONCEPT c
  join @cdm_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (2213466,46275151)
  and c.invalid_reason is null
...
( 
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (2213474,46275143,46275140)and invalid_reason is null
UNION  select c.concept_id
  from @cdm_database_schema.CONCEPT c
  join @cdm_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (2213474,46275143,46275140)
  and c.invalid_reason is null
...
WHERE C.ordinal = 1
AND YEAR(C.drug_exposure_start_date) - P.year_of_birth <= 3
...
) A on A.person_id = P.person_id and A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,-42,P.START_DATE) and A.START_DATE <= DATEADD(day,-1,P.START_DATE)
GROUP BY p.person_id, p.event_id
HAVING COUNT(A.TARGET_CONCEPT_ID) = 0
-- End Correlated Criteria
