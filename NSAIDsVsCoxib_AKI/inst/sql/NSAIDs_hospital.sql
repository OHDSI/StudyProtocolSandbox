...
( 
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (3045262)and invalid_reason is null
...
( 
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (46271022,198185)and invalid_reason is null
UNION  select c.concept_id
  from @cdm_database_schema.CONCEPT c
  join @cdm_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (46271022,198185)
  and c.invalid_reason is null
...
WHERE DATEADD(day,0,OP.OBSERVATION_PERIOD_START_DATE) <= P.START_DATE AND DATEADD(day,30,P.START_DATE) <= OP.OBSERVATION_PERIOD_END_DATE
-- End Primary Events
...
where m.measurement_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 3)
) C
