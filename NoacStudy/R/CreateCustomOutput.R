...
createCustomOutput <- function(outputFolder) {
  ...
  for (drugComparatorOutcome in drugComparatorOutcomesList) {
    # Create PS plots
    psFile <- outcomeReference[outcomeReference$targetId == drugComparatorOutcome$targetId & outcomeReference$comparatorId == drugComparatorOutcome$comparatorId & outcomeReference$sharedPsFile != "", "sharedPsFile"][1]
    ps <- readRDS(psFile)
    plotName <- paste("ps_t", drugComparatorOutcome$targetId, "_c", drugComparatorOutcome$comparatorId, ".png", sep = "")
    ...
    for (analysisId in unique(analysisSummary$analysisId)) {
      ...
      outcomeReferenceSubset <- outcomeReference[outcomeReference$analysisId == analysisId & 
                                                   outcomeReference$targetId == drugComparatorOutcome$targetId &
                                                   outcomeReference$comparatorId == drugComparatorOutcome$comparatorId, ]
      ...
      for (outcomeId in hoiSubset$outcomeId) {
        balanceFile <- outcomeReferenceSubset$covariateBalanceFile[outcomeReferenceSubset$outcomeId == outcomeId]
        if (balanceFile != ""){
          balance <- readRDS(balanceFile)    
          plotName <- paste("balScatter_a",analysisId, "_t", drugComparatorOutcome$targetId, "_c", drugComparatorOutcome$comparatorId, "_o", outcomeId, ".png", sep = "")
          CohortMethod::plotCovariateBalanceScatterPlot(balance, fileName = file.path(outputFolder, plotName))
          plotName <- paste("balTop_a",analysisId, "_t", drugComparatorOutcome$targetId, "_c", drugComparatorOutcome$comparatorId, "_o", outcomeId, ".png", sep = "")
          CohortMethod::plotCovariateBalanceOfTopVariables(balance, fileName = file.path(outputFolder, plotName))
        }
      }
      ...
      for (outcomeId in hoiSubset$outcomeId) {
        ...
        if (om$modelType == "cox") {
          plotName <- paste("km_a",analysisId, "_t", drugComparatorOutcome$targetId, "_c", drugComparatorOutcome$comparatorId, "_o", outcomeId, ".png", sep = "")
      ...
      tableName <- paste("table_a",analysisId, "_t", drugComparatorOutcome$targetId, "_c", drugComparatorOutcome$comparatorId, ".csv", sep = "")      
