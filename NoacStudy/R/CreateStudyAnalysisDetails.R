...
#' @param studyCohortDefinitionTable  The table in the work database schema containing the definitions of the cohorts.
...
createAnalysesDetails <- function(outputFolder) {
  ...
  #   cohortDefSql <- "SELECT cohort_definition_id, cohort_definition_name, cohort_type FROM @target_database_schema.@target_cohort_definition_table"
  #   cohortDefSql <- SqlRender::renderSql(cohortDefSql, target_database_schema=workDatabaseSchema, target_cohort_definition_table=studyCohortDefinitionTable)$sql
  #   cohortDefSql <- SqlRender::translateSql(cohortDefSql, targetDialect = connectionDetails$dbms)$sql
  #   cohortId <- querySql(connection, cohortDefSql)
  #   exposureCohortId <- cohortId[cohortId$COHORT_TYPE == 0,]$COHORT_DEFINITION_ID
  #   outcomeCohortId <- cohortId[cohortId$COHORT_TYPE == 1,]$COHORT_DEFINITION_ID
  #   negativeControlCohortId <- cohortId[cohortId$COHORT_TYPE == 2,]$COHORT_DEFINITION_ID
  #get excluded concepts - any descendants of target/comparator concepts
  #   excludedConceptsql <- paste("SELECT descendant_concept_id FROM @cdmDatabaseSchema.concept_ancestor WHERE ancestor_concept_id IN (SELECT concept_id FROM @cdmDatabaseSchema.concept WHERE concept_name IN ('rivaroxaban','warfarin', 'apixaban', 'dabigatran etexilate'))")
  #   excludedConceptsql <- SqlRender::renderSql(excludedConceptsql, cdmDatabaseSchema = cdmDatabaseSchema)$sql
  #   excludedConceptsql <- SqlRender::translateSql(excludedConceptsql, targetDialect = connectionDetails$dbms)$sql
  #   excludedConcepts <- querySql(connection, excludedConceptsql)
  ...
  rivaroxabanWarfarin <- CohortMethod::createDrugComparatorOutcomes(targetId = 1,
                                                                    ...
                                                                    outcomeIds = c(outcomeCohortId, negativeControlCohortId)                                   
  )
  ...
  matchOnPsVariableArgs <- CohortMethod::createMatchOnPsArgs(caliper=0.2, caliperScale="standardized",maxRatio = 100)
  matchOnPs1to1Args <- CohortMethod::createMatchOnPsArgs(caliper=0.2, caliperScale="standardized",maxRatio = 1)
  stratifyByPsArgs <- CohortMethod::createStratifyByPsArgs(numberOfStrata = 5)
  
  
  #first, crude analyses focused on As-treated vs. ITT, and varying time-at-risk windows (1-30, 1-loe+7, 3-loe+7)
  ...
  #now work through adjustment strategies:
  # PS 1to1 matching;  PS variable matching ; PS 5strata; PS 1to1 match post-trim; 1to1 matching fulloutcome
  ...
  cmAnalysisList <- list(
    #crude analyses:
    cmAnalysis1, cmAnalysis2, cmAnalysis3, cmAnalysis4, cmAnalysis5, cmAnalysis6,
    #adjusted: as-treated, 1-7d
    cmAnalysis7, cmAnalysis8, cmAnalysis9, cmAnalysis10, cmAnalysis11,
    #adjusted: ITT, 1-end
    cmAnalysis12, cmAnalysis13, cmAnalysis14, cmAnalysis15, cmAnalysis16, 
    #adjusted: as-treated, 1-30d, 30d-loe+7d 
    cmAnalysis17, cmAnalysis18
  )
