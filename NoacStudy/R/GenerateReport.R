generateReport <- function(outputFolder) {
  ...
  outcomeReferenceSubset <- outcomeReference[outcomeReference$analysisId == analysisId & outcomeReference$targetId == targetId & outcomeReference$comparatorId == comparatorId,]
  omData <- readRDS(outcomeReferenceSubset$outcomeModelFile[outcomeReferenceSubset$outcomeId == 14])
  resultsSubset <- results[results$analysisId == analysisId & results$targetId == targetId & results$comparatorId == comparatorId,]
  ...
  output <- data.frame(Variable = c("Mean", "Precision"), Estimate = c(null[1], null[2]), lb95 = c(lb95Mean, lb95Precision) , ub95 = c(ub95Mean, ub95Precision))
  ...
  plot <- EmpiricalCalibration::plotCalibrationEffect(negControlSubset$logRr, negControlSubset$seLogRr)
  report <- ReporteRs::addPlot(report, fun = print, x = plot)
  ...
  countVars <- c(10:30, 8507, 8532, 36703451251, 35205243251, 37604050251, 35205189251, 321052251, 4029305251, 198124251, 37219804251, 35909501251, 438112251, 439777251, 21601784554, 21601782554, 21601664554, 21601744554, 1326303504, 21601461554, 21601855554, 21600081554, 21600095554, 21603638554, 21604709551, 21600714553, 21600722552, 21600735554, 1503297504, 21600749554)
  ...
  countBal$countTreatedBefore <- paste(countBal$beforeMatchingSumTreated, " (", round(countBal$beforeMatchingMeanTreated * 100, 1), ")", sep = "") 
  countBal$countComparatorBefore <- paste(countBal$beforeMatchingsumComparator, " (", round(countBal$beforeMatchingMeanComparator * 100, 1), ")", sep = "") 
  countBal$beforeMatchingStdDiff <- round(countBal$beforeMatchingStdDiff, 2)
  countBal$countTreatedAfter <- paste(countBal$afterMatchingSumTreated, " (", round(countBal$afterMatchingMeanTreated * 100, 1), ")", sep = "") 
  countBal$countComparatorAfter <- paste(countBal$afterMatchingSumComparator, " (", round(countBal$afterMatchingMeanComparator * 100, 1), ")", sep = "") 
  countBal$afterMatchingStdDiff <- round(countBal$afterMatchingStdDiff, 2)
  countBal <- countBal[,c("shortName", "countTreatedBefore", "countComparatorBefore" ,"beforeMatchingStdDiff", "countTreatedAfter", "countComparatorAfter", "afterMatchingStdDiff")]
  ...
  meanBal$shortName <- sub(", using conditions all time on or prior to cohort index", "", meanBal$covariateName)
  ...
  meanBal <- meanBal[,c("shortName", "meanTreatedBefore", "meanComparatorBefore" ,"beforeMatchingStdDiff", "meanTreatedAfter", "meanComparatorAfter", "afterMatchingStdDiff")]
  ...
  table <- table[,c("cohortDefinitionName","eventsTreated","treatedRate","eventsComparator","comparatorRate", "rr","ci95lb","ci95ub", "p", "calibratedP")]
  ignore <- table$eventsTreated < 10 | table$eventsComparator < 10
  colnames(table) <- c("Outcome","Riva - # events","Riva - Rate","Warf - # events","Warf - Rate","HR","95% LB","95% UB", "P", "Calibrated P")
