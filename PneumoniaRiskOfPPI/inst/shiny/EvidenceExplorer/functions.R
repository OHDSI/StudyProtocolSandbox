plotBalance <- function(balance, beforeLabel = "Before matching", afterLabel = "After matching") {
...
plotForest <- function(subset, row) {
  ...
  highlight <- subset[subset$targetId == row$targetId & subset$comparatorId == row$comparatorId & subset$outcomeId ==
    row$outcomeId & subset$analysisId == row$analysisId & subset$database == row$database, ]
  plot <- ggplot2::ggplot(subset, ggplot2::aes(x = rr,
                                               y = displayOrder,
                                               xmin = ci95lb,
                                               xmax = ci95ub,
                                               fill = description), environment = environment()) +
          ...
          ggplot2::geom_errorbarh(height = 0.2, size = 1, alpha = 0.7, aes(colour = description)) +
          ggplot2::geom_point(shape = 16, size = 3, alpha = 0.7, aes(colour = description)) +
          ...
          ggplot2::labs(color = "description", fill = "description") +
          ggplot2::theme(text = ggplot2::element_text(size = 15),
