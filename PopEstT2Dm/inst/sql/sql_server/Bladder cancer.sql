...
( 
  select concept_id from @cdm_database_schema.CONCEPT where concept_id in (200680)and invalid_reason is null
...
) A on A.person_id = P.person_id and A.START_DATE BETWEEN P.OP_START_DATE AND P.OP_END_DATE AND A.START_DATE BETWEEN DATEADD(day,1,P.START_DATE) and DATEADD(day,90,P.START_DATE)
GROUP BY p.event_id
