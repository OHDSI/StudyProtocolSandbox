...
target_cohort_table <- "<>"
target_cohort_id <- 389          # 변경필요
comparator_cohort_id <-387        # 변경필요
outcome_cohort_id <- 388         # 변경필요
cdmVersion <- "5" 
...
(
  ...
  WHERE C.drug_exposure_start_date > DATEFROMPARTS(2011, 12, 31)
...
SELECT 0 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
( 
select concept_id from @cdm_database_schema.CONCEPT where concept_id in (75053)and invalid_reason is null
UNION  select c.concept_id
from @cdm_database_schema.CONCEPT c
join @cdm_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
