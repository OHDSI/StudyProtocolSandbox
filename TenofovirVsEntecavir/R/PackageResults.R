...
#
# This file is part of TenofovirVsEntecavir
#
...
packageResults <- function(connectionDetails, cdmDatabaseSchema, outputFolder, minCellCount = 5) {
  ...
  cmAnalysisList <- CohortMethod::loadCmAnalysisList(system.file("settings", "cmAnalysisList.json", package = "TenofovirVsEntecavir"))
  ...
  pathToCsv <- system.file("settings", "TcosOfInterest.csv", package = "TenofovirVsEntecavir")
...
createMetaData <- function(connectionDetails, cdmDatabaseSchema, exportFolder) {
  ...
  snapshot <- OhdsiRTools::takeEnvironmentSnapshot("TenofovirVsEntecavir")
  lines <- c(lines, "")
