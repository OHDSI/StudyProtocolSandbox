...
packageResults <- function(connectionDetails, cdmDatabaseSchema, outputFolder, minCellCount = 5) {
  ...
  cmAnalysisList <- CohortMethod::loadCmAnalysisList(system.file("settings", "cmAnalysisList.json", package = "TofaRep"))
  ...
  pathToCsv <- system.file("settings", "TcosOfInterest.csv", package = "TofaRep")
...
createMetaData <- function(connectionDetails, cdmDatabaseSchema, exportFolder) {
  ...
  snapshot <- OhdsiRTools::takeEnvironmentSnapshot("TofaRep")
  lines <- c(lines, "")
