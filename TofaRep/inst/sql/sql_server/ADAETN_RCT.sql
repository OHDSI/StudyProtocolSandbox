...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (1151789)and invalid_reason is null
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (1119119,1151789)and invalid_reason is null
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (1119119,1151789)
  and c.invalid_reason is null
...
) A on A.person_id = P.person_id and A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE and A.START_DATE <= DATEADD(day,0,P.START_DATE) AND A.END_DATE >= DATEADD(day,1,P.START_DATE) AND A.END_DATE <= P.OP_END_DATE
GROUP BY p.person_id, p.event_id
HAVING COUNT(A.TARGET_CONCEPT_ID) >= 1
-- End Correlated Criteria
...
where o.observation_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 11)
) C
...
(
  select o.*, row_number() over (PARTITION BY o.person_id ORDER BY o.observation_date, o.observation_id) as ordinal
  FROM @cdm_database_schema.OBSERVATION o
where o.observation_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 12)
) C
...
(
  select o.*, row_number() over (PARTITION BY o.person_id ORDER BY o.observation_date, o.observation_id) as ordinal
  FROM @cdm_database_schema.OBSERVATION o
where o.observation_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 16)
) C
...
(
  select po.*, row_number() over (PARTITION BY po.person_id ORDER BY po.procedure_date, po.procedure_occurrence_id) as ordinal
  FROM @cdm_database_schema.PROCEDURE_OCCURRENCE po
where po.procedure_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 16)
) C
...
FROM @cdm_database_schema.DRUG_EXPOSURE de
WHERE (de.drug_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 21) 
	or de.drug_source_concept_id in (SELECT concept_id from  #Codesets where codeset_id = 21))
