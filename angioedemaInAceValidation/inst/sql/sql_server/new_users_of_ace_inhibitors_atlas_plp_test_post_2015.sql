...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (1335471,1340128,19050216,1341927,1342001,1363749,19122327,1308216,1310756,1373225,1331235,1334456,19040051,1342439,19102107)and invalid_reason is null
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (1335471,1340128,19050216,1341927,1342001,1363749,19122327,1308216,1310756,1373225,1331235,1334456,19040051,1342439,19102107)
...
WHERE C.drug_exposure_start_date > DATEFROMPARTS(2015, 01, 01)
...
WHERE C.ordinal = 1
AND C.drug_exposure_start_date > DATEFROMPARTS(2015, 01, 01)
-- End Drug Exposure Criteria
