...
from @results.achilles_results e
join --query below is to compute totals for each stratum
  (select e.analysis_id,e.stratum_1,sum(count_value) as denom from @results.achilles_results e where analysis_id in (1807) group by e.analysis_id,e.stratum_1) s
