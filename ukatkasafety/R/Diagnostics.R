...
generateDiagnostics <- function(outputFolder,
  ...
  analysisSummary <- read.csv(file.path(outputFolder, paste0("analysisSummary", timeAtRiskLabel, ".csv")))
  ...
  subsets <- split(reference, list(reference$targetId, reference$comparatorId, reference$analysisId), drop = TRUE)
  # subset <- subsets[[1]]
