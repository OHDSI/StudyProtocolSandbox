...
exportResults <- function(outputFolder,
                          ...
                          maxCores,
                          timeAtRiskLabel) {
  exportFolder <- file.path(outputFolder, paste0("export", timeAtRiskLabel))
  ...
  exportAnalyses(outputFolder = outputFolder,
                 ...
                 timeAtRiskLabel = timeAtRiskLabel)
...
exportAnalyses <- function(outputFolder,
  ...
  reference <- readRDS(file.path(outputFolder, paste0("cmOutput", timeAtRiskLabel), "outcomeModelReference.rds"))
  getCovariateAnalyses <- function(cmDataFolder) {
    cmData <- CohortMethod::loadCohortMethodData(file.path(outputFolder, paste0("cmOutput", timeAtRiskLabel), cmDataFolder), readOnly = TRUE)
...
exportExposures <- function(outputFolder, exportFolder) {
  ...
  pathToCsv <- system.file("settings", "CohortsToCreate.csv", package = "UkaTkaSafetyFull")
  ...
  createExposureRow <- function(exposureId) {
    ...
    cohortFileName <- system.file("cohorts", paste0(name, ".json"), package = "UkaTkaSafetyFull")
...
exportOutcomes <- function(outputFolder, 
  ...
  synthesisSummaryFile <- file.path(outputFolder, paste0("SynthesisSummary", timeAtRiskLabel, ".csv"))
...
exportMetadata <- function(outputFolder,
  ...
  getInfo <- function(row) {
    cmData <- CohortMethod::loadCohortMethodData(file.path(outputFolder, paste0("cmOutput", timeAtRiskLabel), row$cohortMethodDataFolder), skipCovariates = TRUE)
  ...
  reference <- split(reference, reference$cohortMethodDataFolder, drop = TRUE)
  ...
  for (i in 1:nrow(reference)) {
    outcomeModel <- readRDS(file.path(outputFolder,
                                      paste0("cmOutput", timeAtRiskLabel),
...
exportMainResults <- function(outputFolder,
  ...
  analysesSum <- read.csv(file.path(outputFolder, paste0("analysisSummary", timeAtRiskLabel, ".csv")))
  ...
  subsets <- split(analysesSum,
                   paste(analysesSum$targetId, analysesSum$comparatorId, analysesSum$analysisId), drop = TRUE)
...
exportDiagnostics <- function(outputFolder,
  ...
  preparePlot <- function(i, reference, timeAtRiskLabel) {
  ...
  cluster <- ParallelLogger::makeCluster(min(4, maxCores))
  tasks <- split(reference, seq(nrow(reference)), drop = TRUE)
