...
synthesizePositiveControls <- function(connectionDetails,
  ...
  synthesisFolder <- file.path(outputFolder, paste0("positiveControlSynthesis", timeAtRiskLabel))
  ...
  if (!file.exists(synthesisSummaryFile)) {
    ...
    pathToJson <- system.file("settings", paste0("positiveControlSynthArgs", timeAtRiskLabel, ".json"), package = "UkaTkaSafetyFull")
  ...
  write.csv(allControls, file.path(outputFolder, paste0("AllControls", timeAtRiskLabel, ".csv")), row.names = FALSE)
}
